...
### 容错机制
容错机制指系统在出现异常情况时，可以通过一定的策略保证系统仍然文档运行，从而提高系统的可靠性和健壮性
一般在系统出现错误时触发

**容错策略**
Fail-Over 故障转移：一次调用失败后，切换一个其他节点再次进行调用，也算是一种重试
Fail-Back 失败自动恢复：系统的某个功能出现调用失败或错误时，通过其他方法 恢复该功能的正常，比如降级、重试、调用其他服务
Fail-Safe 静默处理：系统出现部分非功能异常时，直接忽略掉，不做任何处理，就像错误没有发生一样
Fail-Fast 快速失败：系统出现调用错误时，立刻报错，交给外层调用处理

**容错实现方式**
重试：本质上也是一种容错机制，系统错误后再试一次
限流：当系统压力过大、已经出现部分错误时，通过限制执行操作的频率或者数量 对系统进行保护
降级：系统出现错误后，改为执行其他更稳定可用的操作
熔断：系统出现故障或异常时，暂时中断对该服务的请求，而是执行其他操作，以避免连锁故障
超时故障：如果请求或操作长时间没处理完成，就进行中断，防止阻塞和资源占用

结合实际项目，采用多种方式结合来实现容错
由于之前实现了重试，所以也算添加了一种容错机制
在RPC框架中，暂且采用两种方案
1、先容错再重试
系统发生异常时，首先触发容错机制，比如记录日志、进行警告等然后选择是否进行重试
2、先重试再容错
在发生错误时，首先尝试重试机制，如果重试多次失败，则触发容错机制，比如记录日志、进行告警等
可以结合使用

实现两种最基本的容错策略：Fail-Fast快速失败、Fail-Safe静默处理
新建fault.tolerant包，将所有容错的代码放到其下
需要一个容错接口，定义一个容错方法，由于容错是应用到发送请求操作，所以容错方法的返回值是RpcResponse响应对象
然后其具体的实现类去实现这个接口，编写自己的方法去实现响应逻辑（后续扩展）

定义一个容错策略常量类 定义容策策略键名，定义容策工厂
容策策略和注册中心哪些也是一样支持扩展，所以使用工厂模式和SPI机制来加载指定资源的容策策略对象
RpcConfig配置类需要添加策略属性默认为快速失败策略

在ServiceProxy代理请求 发送TCP请求时进行容错处理，trycatch捕捉异常
当请求异常或者失败时，触发容错机制

后续可作其他扩展

### 启动机制和注解驱动
优化框架的易用性，通过建立合适的启动机制和注解驱动机制，帮助开发者最少只用一行代码，就能轻松使用框架

**启动机制**
在Rpc框架中新建bootstrap包，所有和框架启动初始化相关的代码都放到该包下
新建了一个ServiceRegisterIngo 服务注册信息类 包含服务名称和实现类，
在bootstrap包下，创建ProviderBootstrap和ConsumerBootstrap类 用于初始化服务提供者和服务消费者
在服务提供者和服务消费者的代码中 改为调用这两个初始化类来初始化各自的Rpc框架 

**注解驱动**
为了不使代码混淆，新建项目用来实现SpringBootStarter注解驱动的RPC框架

**定义注解**
实现注解驱动的第一步是定义注解，如何定义？ 定义哪些注解？ 参考知名框架Dubbo的注解

在新项目中新建一个annotation包，将所有注解的包放到该包下
在新项目中新建一个bootstrap包，并且分别针对上面定义的3个注解新建启动类

新建两个SpringBoot项目 用于测试
